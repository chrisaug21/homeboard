...
3. **To-do List** — shared household tasks with open count and next-up summary
   Display-only completions trigger one of seven built-in celebration animations before the card clears. `canvas-confetti` powers Confetti Burst, Star Shower, and Fireworks; GSAP powers Sparkle Trail, Bubble Float, and Thumbs Up Bounce; Rainbow Flash stays CSS/JS only

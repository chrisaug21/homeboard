...
- The display footer nav active colors should be controlled per scheme via `--display-nav-active-bg` and `--display-nav-active-border`; Warm and Slate can keep their current accent treatment, while Dark should use a softer low-opacity amber treatment so the active state does not overpower the screen

...
- The display footer nav active colors should be controlled per scheme via `--display-nav-active-bg` and `--display-nav-active-border`; after the token audit, `--display-nav-active-bg` should resolve to `--color-accent-subtle` in every scheme so the component uses the semantic accent token instead of a legacy alias

...
- Display footer nav sizing should be controlled through the shared CSS custom properties `--display-nav-button-width`, `--display-nav-button-active-width`, `--display-nav-button-height`, `--display-nav-button-gap`, and `--display-nav-button-radius`
- The display footer nav active fill must always use the same theme token as the footer Homeboard label (`var(--amber)` here), never a hardcoded hex color, and it must resolve correctly in Warm, Dark, and Slate

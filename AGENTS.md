...
- The display footer nav active colors are controlled per scheme via `--display-nav-active-bg` and `--display-nav-active-border`; Warm and Slate can track their current accent treatment, while Dark should use a softer low-opacity amber treatment so the active state stays subtle

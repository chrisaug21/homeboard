...
- The display footer nav active colors are controlled per scheme via `--display-nav-active-bg` and `--display-nav-active-border`; after the token audit, `--display-nav-active-bg` should point to `--color-accent-subtle` in every scheme so the component consumes the semantic accent token rather than a scheme-specific legacy alias

...
- The display footer nav active fill must always come from the current theme token (`var(--amber)` in this app), never a hardcoded hex value, so Warm, Dark, and Slate stay visually consistent

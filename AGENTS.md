...
- Display footer nav sizing should be tuned via the shared CSS custom properties `--display-nav-button-width`, `--display-nav-button-active-width`, `--display-nav-button-height`, `--display-nav-button-gap`, and `--display-nav-button-radius`
- The display footer nav active fill must always come from the same theme token that colors the footer Homeboard label (`var(--amber)` in this app), never a hardcoded hex value, and it must resolve correctly in Warm, Dark, and Slate
